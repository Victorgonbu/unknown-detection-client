...
const server = setupServer(
  rest.get('http://localhost/api/v1/posts', (req, res, ctx) => res(ctx.json(
    postsResponse,
  ))),
  rest.get('http://localhost/api/v1/favorites', (req, res, ctx) => res(ctx.json(
    postsResponse,
  ))),
...
describe('Posts', () => {
  ...
  describe('render errors', () => {
    it('when unable to hit API', async () => {
      server.use(rest.get('http://localhost/api/v1/posts', (req, res, ctx) => res(ctx.status(500))));
    ...
    it('when no matches are retrieved from search', async () => {
      server.use(rest.get('http://localhost/api/v1/posts', (req, res, ctx) => res(
        ctx.status(404),
        ctx.json({
